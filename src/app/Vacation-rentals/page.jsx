import { SearchBarPc } from "@/components/SearchBarFormPc/SearchBarPc";
import { IoHomeOutline, IoLocationOutline } from "@/utils/Icons";
import { CardSkeleton } from "@/Shared/Skeletons/CardSkeleton";
import { BgGlassmorphism } from "@/Shared/BgGlassmorphism";
import { Heading } from "@/Shared/Headings/Heading";
import supabase from "@/utils/Supabase";
import { Suspense, cache } from "react";
import dynamic from "next/dynamic";
import Image from "next/image";

const Stays = dynamic(() => import('@/Shared/Stays'), { loading: () => <CardSkeleton /> })

export const metadata = {
  title: 'Vacation rentals',
  alternates:{
    canonical:"/Vacation Rentals"
  },
}

const getData = cache(async () => {
  const { count } = await supabase.from("House").select("*", { count: "exact" });

  const totalCount = count

  return { totalCount };
});

export const revalidate = 60;

export async function ListingStayPage({ searchParams }) {

  const { totalCount } = await getData();

  const page = searchParams["page"] ?? 1;

  return (
    <main className="container">
      <BgGlassmorphism />
      <div className="space-y-36">
        <Hero totalCount={totalCount} />
        <section id="Stays" className={`pb-24 lg:pb-28`}>
          <Heading
            tagName="h2"
            title="Stays in Nosara"
            paragraph={`Discover populars house to stay in Nosara`}
          />
          <Suspense fallback={<CardSkeleton />}>
            <Stays page={page} totalCount={totalCount} />
          </Suspense>
        </section>
      </div>
    </main>
  );
}

function Hero({totalCount}) {
  return <section className={`flex flex-col relative`}>
  <div className="flex flex-col lg:flex-row lg:items-center  pt-10 lg:pt-16 lg:pb-16">
    <div className="flex-shrink-0 lg:w-1/2 flex flex-col items-start space-y-6 lg:space-y-10 pb-14 lg:pb-64 xl:pb-80 xl:pr-14 lg:mr-10 xl:mr-0">
      <h1 className="font-medium text-5xl !leading-[114%] max-[355px]:text-4xl md:text-6xl xl:text-7xl">
        Discover best stays in Nosara
      </h1>
      <div className="flex items-center text-base md:text-lg text-gray-500">
        <IoLocationOutline className="text-[1.5rem]" />
        <span className="ml-2.5">Costa Rica</span>
        <span className="mx-5"></span>
        <IoHomeOutline className="text-[1.5rem]" />
        <span className="ml-2.5">{totalCount} houses</span>
      </div>
    </div>
    <div className="flex-grow">
      <Image
        src={"/Hero-Stays.webp"}
        width={1001}
        height={1031}
        alt="Hero Image"
        priority
      />
    </div>
  </div>

  <div className="hidden lg:flow-root w-full">
    <div className="z-10 lg:-mt-40 xl:-mt-56 w-full">
      <SearchBarPc />
    </div>
  </div>
</section>
}

export default ListingStayPage;
