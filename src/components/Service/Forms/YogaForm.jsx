export function YogaForm({ Pax, Yoga, Day, Information, updateFields, isFirstStep }) {
  return (
    <>
      {isFirstStep && (
        <>
          <div className="mb-5">
            <label htmlFor="yoga" className="mb-3 block text-sm font-medium text-gray-900" >
            Yoga service name
            </label>
            <input type="text" name="yoga" id="yoga" placeholder="Reiki with sounds" className="w-full appearance-none rounded-md border border-[#e0e0e0] bg-white py-3 px-3 text-sm outline-none focus:border-[#121213] focus:shadow-md" required value={Yoga} onChange={(e) => updateFields({ Yoga: e.target.value })} />
          </div>
