  ...
  const renderWidgetMenuItem = (menu, index) => {
    return (
      <div key={index} className="text-sm">
        <span className="font-semibold text-dark">{menu.title}</span>
        <ul className="mt-5 space-y-4">
          {menu.menus.map((item, index) => (
            <li key={index}>
              {item.url ? (
                <Link key={index} href={item.url} className="hover:text-dark">
                  {" "}
                  {item.name}{" "}
                </Link>
              ) : (
                <span>
                  <HiOutlineInformationCircle className="relative -top-0.5 inline text-base" />{" "}
                  {item.label}
                </span>
              )}
            </li>
